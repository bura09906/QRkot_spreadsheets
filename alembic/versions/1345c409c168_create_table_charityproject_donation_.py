"""Create table CharityProject, Donation, User

Revision ID: 1345c409c168
Revises: 
Create Date: 2024-10-28 00:17:20.469321
...
def upgrade():
    ...
    sa.CheckConstraint('length(name) >= 1 AND length(description) >= 1', name='Проверка минимального значения полей name и description'),
    ...
    sa.CheckConstraint('full_amount > 0 AND invested_amount >= 0', name='Проверка целочисленных полей'),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_donation_close_date'), 'donation', ['close_date'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_donation_close_date'), table_name='donation')
